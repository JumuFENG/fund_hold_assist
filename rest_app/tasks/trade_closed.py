...
def save_earning_task():
    ...
    while retry < mxretry:
        try:
            TimerTask.logger.info(f'save_earning_task!')
            ...
            retry += 1
            TimerTask.logger.error(f'Error saving earnings: {e}')
...
class SaveEarningTask(TimerTask):
    def __init__(self) -> None:
        super().__init__('15:02:03', save_earning_task)
...
def update_bkchanges_history():
    try:
        ...
        TimerTask.logger.error(f'Error updating bk changes history: {e}')
        TimerTask.logger.error(format_exc())


class BkChagnesHistoryTask(TimerTask):
    def __init__(self) -> None:
        super().__init__('15:01:04', update_bkchanges_history)


def update_daily_trade_closed_history():
    try:
        AllIndexes.update_kline_data('d')
        TradingDate.clear_cache()
        AllStocks.update_kline_data('d')
    except Exception as e:
        TimerTask.logger.error(f'Error updating daily history data: {e}')
        TimerTask.logger.error(format_exc())


class TradeClosedHistroyDataTask(TimerTask):
    def __init__(self) -> None:
        super().__init__('15:01:20', update_daily_trade_closed_history)
...
if __name__ == '__main__':
    ...
    tasks = [SaveEarningTask(), BkChagnesHistoryTask(), SmStatsTask1501(), TradeClosedHistroyDataTask()]
