...
let strategyOptions = {'zt1': {name:'首板次日买入', val:'zt1', strategy:'StrategyZt1'}, 'dt3': {name:'连续跌停', val:'dt3'}}
let mktOptions = {'all': {name:'全部', val:0}, 'main': {name:'沪深主板', val: 1}, 'part': {name:'双创', val: 2}}
...
class StockData {
        ...
        if (this.klchecked[code] === undefined || sdate < this.klchecked[code]) {
            ...
            if (!emjyBack.klines[code]) {
                emjyBack.loadKlines(code, _ => {
                    if (!emjyBack.klines[code] || !emjyBack.klines[code].getKlineByTime(sdate, kltype)) {
                        emjyBack.fetchStockKline(code, kltype, sdate);
                    }
                });
            } else if (!emjyBack.klines[code].getKlineByTime(sdate, kltype)) {
    ...
    retro(stkTable, mkt) {
        var mktstocks = this.stocks.filter(x => {
