...
class DbkRequests {
    ...
    getRanks(cb) {
        var url = this._redirect ? '/api/get?url=' + this.rankUrl : this.rankUrl;
        utils.get(url, null, response => {
    ...
    onRankBack(rankhtml, cb) {
        ...
        for (let i = 0; i < rows.length && i < 100; i++) {
            ...
            ranks.push({code: cels[0].innerText.trim().split(' ')[0], rank: cels[2].innerText.trim()});
        }
    ...
    getZdt(cb) {
        var url = this._redirect ? '/api/get?url=' + this.ztUrl : this.ztUrl;
        utils.get(url, null, response => {
    ...
    getYzdt(cb) {
        var url = this._redirect ? '/api/get?url=' + this.yzUrl : this.yzUrl;
...
var dbkCommon = new DbkRequests(true);
