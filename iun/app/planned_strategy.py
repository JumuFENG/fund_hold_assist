...
class StrategyGE(PlannedStrategy):
    ...
    async def check_kline(self, acc, code, kltypes):
        buydetails = StrategyFac.get_buy_details(acc, code)
        smeta = StrategyFac.get_strategy_meta(acc, code, self.key)
        if len(buydetails) > 0 and ('guardPrice' not in smeta or float(smeta['guardPrice']) < FnPs.min_buy_price(buydetails)):
            smeta['guardPrice'] = FnPs.min_buy_price(buydetails)

        lkltype = smeta['kltype']
            ...
            if kl1:
                ...
                if smeta['inCritical']:
                    if kl1['close'] - (smeta['guardPrice'] - mxprice * smeta['stepRate'] * 0.8) > 0:
                        ...
                        StrategyFac.update_strategy_meta(acc, code, self.key, smeta)
                        return
        ...
        if kl and len(buydetails) > 0 and lkltype in kltypes and FnPs.bss18_sell_match(kl):
            ...
            count = FnPs.get_sell_count_matched(buydetails, smeta['cutselltype'], kl.c, smeta['stepRate'])
            if count > 0:
                StrategyFac.planned_strategy_trade(acc, code, 'S', kl['close'], count)
                buydetails = FnPs.consume_buy_details(buydetails, count)
                StrategyFac.update_buy_details(acc, code, buydetails)
...
class StrategySellELShort(PlannedStrategy):
    ...
    async def check_quotes(self, acc, code):
        quotes = self.qwatcher.get_quotes(code)
        if not quotes:
            return
        ...
        buydetails = StrategyFac.get_buy_details(acc, code)
        if quotes['high'] == quotes['top_price']:
...
class StrategySellBeforeEnd(PlannedStrategy):
    ...
    def check_kline(self, acc, code, kltypes):
        ...
        if smeta['sell_conds'] & conditions['not_zt']:
            if kl['close'] < ztprice:
                self.dosell(acc, code, kl['close'], count, buydetails, smeta)
                return
    ...
    def dosell(self, acc, code, price, count, buydetails, smeta):
...
class StrategyFac():
    ...
    def get_strategy(self, k):
        if k not in self.__strategies:
            if k == StrategyGE.key:
                self.__strategies[k] = StrategyGE()
            elif k == StrategySellELShort.key:
                self.__strategies[k] = StrategySellELShort()
            elif k == StrategySellBeforeEnd.key:
                self.__strategies[k] = StrategySellBeforeEnd()
            elif k == StrategySellMA.key:
                self.__strategies[k] = StrategySellMA()
            else:
                logger.error('Strategy not implemented: %s', k)
                return None
            asyncio.create_task(self.__strategies[k].start_strategy_tasks())
        return self.__strategies[k] if k in self.__strategies else None

    __strategies_data = {}
    @classmethod
    def cache_strategy_data(self, acc, code, data):
        self.__strategies_data[(acc, code)] = data

    @classmethod
    def get_buy_details(self, acc, code):
        if (acc, code) not in self.__strategies_data:
            return []
        return self.__strategies_data[(acc, code)]['strategies']['buydetail']
    
    @classmethod
    def update_buy_details(self, acc, code, buydetails):
        if (acc, code) not in self.__strategies_data:
            logger.error('Strategy not exist: %s %s', acc, code)
            return None
        self.__strategies_data[(acc, code)]['strategies']['buydetail'] = buydetails

    @classmethod
    def get_strategy_meta(self, acc, code, skey):
        if (acc, code) not in self.__strategies_data:
            return None
        for s in self.__strategies_data[(acc, code)]['strategies']['strategies'].values():
            if s['key'] == skey:
                return s
    ...
    @classmethod
    def planned_strategy_trade(self, acc, code, tradeType, price, count, tacc=None):
        buydetails = StrategyFac.get_buy_details(acc, code)
        tacc = acc if tacc is None else tacc
        if tradeType == 'B':
            if count == 0:
                amount = self.__strategies_data[(acc, code)]['strategies']['amount']
                count = guang.calc_buy_count(amount, price)
            buydetails.append({'code': code, 'count': count, 'price': price, 'date': guang.today_date('-'), 'type': 'B'})
        else:
            buydetails = FnPs.consume_buy_details(buydetails, count)
        # TradeInterface.submit_trade({'account': acc, 'code': code, 'tradeType': tradeType, 'count': count, 'price': price})
        logger.info('Strategy trade: %s %s %s %f %d', acc, code, tradeType, price, count)
        self.update_buy_details(acc, code, buydetails)
